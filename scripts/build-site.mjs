...
for (const line of text
  ...
  const [platform, name, context, weights, features, source] = splitTableRow(line);
  specs.set(modelKey(platform, name), { context, weights, features, source });
}
...
const value = all
  ...
  .sort(sort);
const other = all
  .filter((x) => x.m.category === 'other' && x.m.status !== 'Deprecated')
  .sort(sort);
...
for (const line of routerText
  ...
  const [platform, name, url, price] = splitTableRow(line);
  openrouter.set(modelKey(platform, name), { url, price });
}
...
function routerInfo(p, model) {
  ...
  return (
    openrouter.get(modelKey(p.platform, model.name)) || {
      url:
    ...
    entries
      .map((entry) => {
        ...
        const displayValue = (getter) =>
          getter(m) || '미상';
        const displayLink = (getter) => link(getter(m) || '');
        const spec = specs.get(modelKey(p.platform, m.name)) || {};
        const displaySpecs = (getter) => esc(getter(spec, m) || '미상');
        const displaySpecLinks = () =>
          spec.source
            ? '<details class="source-details"><summary>사양 출처</summary><small>' +
              link(spec.source) +
              '</small></details>'
            : '';
        const modelNames = m.name.replaceAll('`', '');
        ...
        return (
          ...
          '</td><td>' +
          link(m.cost ? m.cost.replaceAll('`', '') : '가격 미상') +
          '<small>' +
          link(m.pricing) +
          '</small>' +
          (routerInfo(p, m)?.price
            ? '<small>' + esc(routerInfo(p, m).price) + '</small>'
            : '') +
          '</td><td>' +
          displayLink((model) => model.intro) +
          displaySpecLinks() +
          (routerInfo(p, m)
            ? '<small>' + link(routerInfo(p, m).url) + '</small>'
            : '') +
          '</td><td>' +
...
const summary =
  '<p class="intro">Artificial Analysis와 제공자 공식 자료를 확인한 수동 모델 인덱스입니다. Context length는 토큰 단위이며 API·앱·설정별 한도가 다를 수 있습니다.</p><nav aria-label="모델 목록"><a href="#frontier">Frontier</a><a href="#value">가성비</a><a href="#other">그외 모델</a><a href="#platforms">플랫폼 타임라인</a></nav>';
const providers =
...
await writeFile(
  ...
  document(
    ...
    summary +
      ...
      frontier.length +
      '</span></h2><p>2026-09-14 확인 스냅샷 기준의 수동 큐레이션 9개입니다. ' +
      link('[Artificial Analysis LLM leaderboard](https://artificialanalysis.ai/leaderboards/models)') +
      '와 제공자 공식 자료를 대조했으며, 실시간 순위를 재현하거나 점수를 저장하지 않습니다.</p>' +
      table(frontier) +
      ...
      table(value) +
      '</section><section id="other"><h2>그외 모델 <span>' +
      other.length +
      '</span></h2><p>언어 제공자별로 공식 자료에서 확인한 현재 대표 모델을 하나씩 기록합니다. Frontier·Value 선택과 중복하지 않으며, 전체 과거 이력은 각 플랫폼 Markdown에서 확인합니다.</p>' +
      table(other) +
      '</section>' +
      providers,
  ),
